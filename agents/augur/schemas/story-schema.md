...
- when `observations/components.json` and `observations/stories.json` are present, use them to challenge root choice and child-story decomposition before writing
- when `observations/narratives.json` is present, keep story scope and teaching value strong enough that narratives can select stories by explanatory value rather than by root coverage alone

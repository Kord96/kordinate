...
def main() -> int:
    ...
    prompt = "\n".join([
        ...
        "When getting-started or other narrative-selection issues remain, use `facts/narrative-seeds.json` when present to challenge which roots, child stories, flow-bearing stories, and boundary stories actually deserve inclusion.",
        "When the introductory path lacks a strong operating model, use `facts/control-hotspots.json` when present to prefer a story that teaches a defining control, request, or execution path.",
        "When the introductory path lacks a real state or dependency boundary, use `facts/state-access-summary.json` when present to prefer a story that explains a concrete storage, state, or external-boundary interaction.",
        ...
        "When a narrative overview is too thin, rewrite `getting-started.description` as a compact architecture synopsis that teaches system shape plus the operating model instead of a generic one-liner or component catalog.",

...
def main() -> int:
    ...
    prompt = "\n".join([
        ...
        "When shared cascades are implicated, use `facts/failure-scenario-candidates.json` to decide whether several unit-level health notes should be replaced by one top-level `failure_scenarios` entry.",
        ...
        "When health blocks are thin, add `health.criteria` so the component, flow, or dependency states what healthy operation looks like before describing how it fails.",
        "Use the atlas component graph while repairing health: parent-child structure tells you ownership, but `depends_on`, shared state, and shared external dependencies should drive seam and cascade modeling more strongly than simple containment.",

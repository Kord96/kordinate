...
def build_prompt(run_dir: Path) -> str:
    ...
    facts_index = load_json(run_dir / "index.json")
    concepts = load_json(run_dir / "facts" / "concepts.json")
    ...
    patterns = [
        ...
        for fact in concepts.get("facts", [])[:10]
...
def collect_for(agent: str, run_dir: Path, prompt: str, discovered: dict[str, dict[str, str]], gateway_url: str, api_key: str, wait_ms: int) -> Path:
    ...
    record = {
        ...
        "source_analysis_meta_path": str(meta_path),
        "source_agent": agent,
        "reflection": {

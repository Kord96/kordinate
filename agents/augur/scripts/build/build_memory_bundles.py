...
def common_header(title: str, mode_note: str) -> list[str]:
    return [
        ...
        '2. Start from the prepared deterministic artifacts for this run: `$RUN/blast.json`, `$RUN/startup.json`, `$RUN/index.json`, `$RUN/facts/`, `$RUN/observations/`, and `$RUN/derived/`.',
        '3. Use deterministic evidence, including `observations/concepts.json`, to decide what deserves attention.',

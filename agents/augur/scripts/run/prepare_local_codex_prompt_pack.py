...
LOCAL_AGENT_HOME = Path(os.environ.get("AUGUR_LOCAL_AGENT_HOME", "/kord/agents/augur-local-codex"))
...
def bootstrap_local_run(project: str, working_dir: Path, analysis_mode: str) -> Path:
    ...
    prepared = subprocess.check_output(
        [
            "python3",
            str(ROOT / "scripts" / "run" / "prepare_analysis_dir.py"),
            str(working_dir),
            "--agent-home",
    ...
    subprocess.check_call([
        "python3",
        str(ROOT / "scripts" / "run" / "prepare_deterministic_run.py"),
        str(working_dir),
        "--run-dir",
        str(run_dir),
        "--project",
...
def resolve_analysis_plan(project: str, working_dir: Path, analysis_mode: str, bundle_mode: str) -> dict:
    return run_json(
        ROOT / "scripts" / "run" / "resolve_analysis_plan.py",
        str(working_dir),
        "--project",
        project,
        "--agent-home",
...
def render_local_runtime_context(context: dict) -> str:
    lines = [
        ...
        "- Treat the output directory above as the authoritative home for generated artifacts such as `startup.json`, `index.json`, `facts/*`, `observations/*`, `derived/*`, `atlas.json`, `stories/`, and `narratives.yaml`.",

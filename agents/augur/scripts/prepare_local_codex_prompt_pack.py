...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--analysis-mode", default="full", choices=["full", "incremental"], help="Prepared semantic analysis mode")
    parser.add_argument("--bundle-mode", default="full-bundle", help="Bundle mode such as selective or full-bundle")
...
def bootstrap_local_run(project: str, working_dir: Path) -> Path:
...
def render_local_runtime_context(context: dict, bundle_mode: str) -> str:
    lines = [
        ...
        f"- Run dir: `{context['run_dir']}`",
        f"- Facts dir: `{context['facts_dir']}`",
        f"- Blast file: `{context['blast_path']}`",
        f"- Startup manifest: `{context['startup_path']}`",
        f"- Bundle mode: `{bundle_mode}`",
        "- This local Codex session is standing in for the semantic agent runtime.",
        "- Treat the run dir above as the authoritative home for generated artifacts such as `facts/*`, `atlas.json`, `stories/`, `narratives.yaml`, and `meta.json`.",
...
def main() -> int:
    ...
    if args.run_dir:
        ...
        try:
            ...
            run_dir = bootstrap_local_run(args.project, working_dir)

    prompt_context = run_json(
        ROOT / "scripts" / "build_prompt_context.py",
        "--bundle-mode", args.bundle_mode,
    ...
    prompt_text = build_prompt(args.project, args.analysis_mode, args.bundle_mode, prompt_context, analysis_context)

...
2. Read `$RUN/facts/startup.json` and use it as the startup manifest for available domains, counts, detector coverage, and failed domains. Open `$RUN/facts/index.json` only when you need the full deterministic manifest.
...
- Treat `facts/startup.json` as the first control surface and `facts/index.json` as the deeper follow-up manifest. Do not read every fact domain wholesale before you know which domains matter.

...
from utils import detector_metadata_from_record, make_entity_ref, normalize_fact_record
...
def main() -> int:
    ...
    normalized_facts = [
        normalize_fact_record({
            "id": stable_id(item["file"], "state"),
            "kind": "state-seed",
            "domain": "state-seeds",
            "summary": f"State grounding inventory for {item['file']}",
            "source_files": [item["file"], *list(item.get("line_refs") or [])],
            "raw_evidence": {**item, "confidence_hint": "medium"},
            "relationships": [
                relation
                for relation in [
                    make_entity_ref("component", str(item.get("component_seed_id") or ""), "grounded_in_component")
                ]
                if relation
            ],
        })

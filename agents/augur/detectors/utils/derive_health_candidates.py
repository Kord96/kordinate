...
from utils import component_ids_from_relationships, detector_metadata_from_record, fact_payload, make_entity_ref, normalize_fact_record
...
def top_component_ids(facts: list[dict[str, Any]]) -> list[str]:
    ...
    seen: set[str] = set()
    for fact in facts:
        for component_id in component_ids_from_relationships(fact.get("relationships")):
...
def main() -> int:
    ...
    local = local_candidates(frameworks, jobs, config, routes)
    integration = integration_candidates(state_access_summary, external_clients, routes, dispatch_bindings)
    propagation = propagation_candidates(state_access_summary, external_clients, control_hotspots)
    facts: list[dict[str, Any]] = []
    for item in local:
        facts.append(normalize_fact_record({
            "id": str(item.get("id") or ""),
            "kind": "health-local-candidate",
            "domain": "health-candidates",
            "summary": str(item.get("rationale") or ""),
            "source_files": list(item.get("evidence_refs") or []),
            "raw_evidence": {**item, "confidence_hint": "medium"},
            "relationships": [relation for relation in [make_entity_ref("component", str(item.get("component") or ""), "grounded_in_component")] if relation],
        }))
    for item in integration:
        integration_relationships = [
            relation
            for relation in [
                make_entity_ref("component", str(item.get("source") or ""), "grounded_in_component"),
                make_entity_ref("component", str(item.get("target") or ""), "related_component"),
            ]
            if relation
        ]
        facts.append(normalize_fact_record({
            "id": str(item.get("id") or ""),
            "kind": "health-integration-candidate",
            "domain": "health-candidates",
            "summary": str(item.get("rationale") or ""),
            "source_files": list(item.get("evidence_refs") or []),
            "raw_evidence": {**item, "confidence_hint": "medium"},
            "relationships": integration_relationships,
        }))
    for item in propagation:
        relationships = [
            ...
            for relation in [
                make_entity_ref("component", str(item.get("source") or ""), "grounded_in_component"),
                *[
                    make_entity_ref("component", str(value), "related_component")
                    for value in (item.get("affects") or [])
                ],
            ]
            if relation
        ]
        facts.append(normalize_fact_record({
            "id": str(item.get("id") or ""),
            "kind": "health-propagation-candidate",
            "domain": "health-candidates",
            "summary": str(item.get("rationale") or item.get("degraded_mode_hint") or ""),
            "source_files": list(item.get("evidence_refs") or []),
            "raw_evidence": {**item, "confidence_hint": "medium"},

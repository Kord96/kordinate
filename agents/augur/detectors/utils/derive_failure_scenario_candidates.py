...
from utils import component_ids_from_relationships, detector_metadata_from_record, fact_kind, fact_payload, make_entity_ref, make_fact_ref, normalize_fact_record
...
def strong_concepts(concept_payload: dict[str, Any]) -> list[dict[str, Any]]:
    selected: list[dict[str, Any]] = []
    for fact in concept_payload.get("facts") or []:
        if not isinstance(fact, dict) or fact_kind(fact) != "concept-candidate":
            continue
        ...
        confidence = str(raw.get("confidence_hint") or "low").strip().lower()
...
def build_candidates(
    ...
    for fact in strong_concepts(concept_payload):
        raw = fact_payload(fact)
        ...
        components = [
            str(item)
            for item in component_ids_from_relationships(fact.get("relationships"))
...
def main() -> int:
    ...
    derived_dir = facts_dir.parent / "derived"
    health_candidates_payload = load_optional_json(facts_dir / "health-candidates.json")
    concept_payload = load_optional_json(facts_dir / "concepts.json")
    monitoring_index = concept_monitoring_index()
    component_seeds_payload = load_optional_json(derived_dir / "component-seeds.json")
    component_seed_ids = {
        str(item.get("id") or "")
    ...
    normalized_facts = [
        normalize_fact_record({
            ...
            "kind": "failure-scenario-candidate",
            "domain": "failure-scenario-candidates",
            "summary": str(item.get("degraded_mode_hint") or item.get("rationale") or ""),
            ...
            "relationships": [
                *[
                    ...
                    for relation in (
                        ...
                        for component_id in (item.get("involves") or [])
                    )
                    if relation
                ],
                *[
                    relation
                    for relation in (
                        make_fact_ref(str(fact_id), "related_to")
                        for fact_id in (item.get("concept_sources") or [])
                    )
                    if relation
                ],
            ],

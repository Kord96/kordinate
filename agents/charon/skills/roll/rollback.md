...
- A rollback snapshot must exist at `$MEM/rollback/<project>-<env>.json`
...
2. **Load snapshot** — read `$MEM/rollback/<project>-<env>.json`. If no snapshot exists, report error: "No rollback snapshot found. Rollback is only available after a /roll." and exit.
...
- Snapshot files live in project memory (`$MEM/rollback/`) — survives pod restarts

...
Usage:
    ...
    e.g., /path/to/agent/memory/projects/<project>/
...
def main():
    ...
    # Expected: memory/projects/<project>/ → project name is used to find root
    project_root = None
    if "projects" in mem_dir.parts:
        ...
        if proj_idx + 1 < len(mem_dir.parts):
            ...
            for candidate in [Path(f"/kord/projects/{project_name}"), Path.home() / project_name]:
                if candidate.exists():

---
description: Deployer working notes and observations
curated: false
scope: global
preloaded: deployer
---

- git-crypt in detached worktree requires: (1) create with --no-checkout, (2) copy symmetric key from existing worktree's git-crypt/keys/default, (3) git reset HEAD, (4) git checkout . — avoids smudge filter failures
- git-crypt rebase workaround: disable smudge/clean filters (set to 'cat', required=false) before rebase, use -X theirs for encrypted file conflicts, then unset filters after
- For future RWX PVC expansions in Longhorn: must restart the share-manager pod in longhorn-system after patching PVC size, then patch PVC status subresource to clear FileSystemResizePending condition
- SSH to clusters: use kkord@ user, not claude@. Tailscale IPs: vandc=100.95.237.24, home=100.71.90.43
- No rsync on workstation pod — use scp for manifest sync
- Gateway consolidated into single pod (alloy, prometheus, loki, tailscale). Gateway Tailscale IPs: gateway-home=100.79.204.119, gateway-vandc=100.93.174.24
- For project-specific notes, use `/kord remember` to save memories.
- Beorn needs to be started locally for development — run `node server.js` from `lib/mcp-agent-server/`
- Beorn PVC worktree lifecycle: entrypoint.sh sets KORDINATE_HOME=/kord-shared (PVC mount), validates .git exists, creates .worktrees/ and .locks/ dirs, prunes stale worktrees on startup, symlinks ~/.kord -> /kord-shared. server.js creates per-agent worktrees (memory/<id> branches), sets KORDINATE_HOME to worktree path for spawned agents, ff-only merges with flock after agent finishes, cleans up. Cache writes (kord data.md) stay on main KORDINATE_HOME.
- Home cluster Longhorn gap (2026-03-26): setup-storage in deploy-cluster.md was docs-only, no backing automation. A fake "longhorn" StorageClass existed using rancher.io/local-path. Longhorn v1.7.3 installed manually, requires open-iscsi on all Linux nodes (colima/macOS can't run it). deploy-cluster.md updated with explicit install steps. kord PVC needs migration from hostPath/RWO to Longhorn/RWX via migrate upgrade-storage.

...
| `~/.claude/projects/<project>/memory/MEMORY.md` | Main session auto memory — Claude writes this itself as it works. First 200 lines auto-loaded at startup. Additional topic files (e.g. `debugging.md`) can exist in the same directory — Claude reads those on-demand when it needs them. |

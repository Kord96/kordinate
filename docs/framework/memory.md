...
All properties live in `MAP.json` — the single registry for all knowledge. Only non-default values need to be specified.
...
Auto-generated by `/scribe:map`. The linker reads `MAP.json` to know every file, its purpose, and how to handle it.

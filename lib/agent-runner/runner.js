...
const GLOBAL_MEMORY_DIR = `${AGENT_PROJECT_DIR}/memory/global`;
...
// Track last known mtime of global memory dir to detect curator updates
let lastGlobalMemoryMtime = 0;

function getGlobalMemoryMtime() {
  try {
    if (!existsSync(GLOBAL_MEMORY_DIR)) return 0;
    let latest = 0;
    for (const f of readdirSync(GLOBAL_MEMORY_DIR)) {
      const mt = statSync(`${GLOBAL_MEMORY_DIR}/${f}`).mtimeMs;
...
function initGlobalMemoryMtime() {
  lastGlobalMemoryMtime = getGlobalMemoryMtime();
}
...
async function processJob(message) {
  ...
  try {
    // Check if global memory was updated since last job
    let memoryRefresh = '';
    const currentMtime = getGlobalMemoryMtime();
    if (currentMtime > lastGlobalMemoryMtime) {
      memoryRefresh = `\nYour global memory has been updated. Read the files in ${GLOBAL_MEMORY_DIR}/ to refresh your knowledge before starting.\n`;
      lastGlobalMemoryMtime = currentMtime;
    }
    ...
    if (status === 'success' && REFLECTION_PROMPT) {
      try {
        const reflectionCtx = job.project
          ? REFLECTION_PROMPT.replace('{{PROJECT}}', job.project)
          : REFLECTION_PROMPT.replace('{{PROJECT}}', '');
        await Promise.race([
...
async function main() {
  ...
  // Snapshot global memory mtime before first job
  initGlobalMemoryMtime();

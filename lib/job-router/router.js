...
// REST delegate endpoint — used by agent pods for inter-agent communication
app.post('/api/delegate', async (req, res) => {
  ...
  if (!KNOWN_AGENTS.includes(agent)) {
    return res.status(404).json({ error: `Unknown agent: ${agent}. Known: ${KNOWN_AGENTS.join(', ')}` });
  }
  log('API delegate', { agent, prompt: prompt.substring(0, 100) });
  try {
    const output = await submitAndWait(agent, prompt);
    res.json({ agent, status: 'success', output });
  } catch (e) {
    log('API delegate error', { agent, error: e.message });
    res.status(500).json({ agent, status: 'error', error: e.message });
  }
});

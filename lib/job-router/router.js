...
// REST delegate endpoint — route jobs to any agent via Kafka topic.
// No registry lookup required: if a consumer listens on jobs.<agent>, the job
// gets processed. Callers can optionally pass project/repo metadata.
app.post('/api/delegate', async (req, res) => {
  ...
  try {
    ...
    if (result.status === 'error' || result.status === 'timeout') {
      return res.status(502).json({ agent, job_id: jobId, correlation_id: correlationId, status: result.status, error: result.output });
    }
    res.json({ agent, job_id: jobId, correlation_id: correlationId, status: 'success', output: result.output });
  } catch (e) {
...
// List known agents by querying Kafka for jobs.* topics
...
async function shutdown() {
  log('Router shutdown');
  try { await resultConsumer.disconnect(); await producer.disconnect(); await admin.disconnect(); } catch {}
  process.exit(0);
}

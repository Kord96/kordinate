      ...
      if (!existsSync(projPath)) {
        if (job.repo) {
          ...
          try {
            ...
            execSync(`git clone --depth 1 ${job.repo} ${projPath}`, { timeout: 120000 });
          } catch (e) {
...
async function main() {
  ...
  // Process one job at a time. Pause consumer while busy so KEDA-scaled
  // pods can pick up remaining messages from the topic.

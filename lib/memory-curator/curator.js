#!/usr/bin/env node
// Memory Curator v2 — consumes memory updates from Kafka, deduplicates via
// Haiku, routes to global or project-scoped agent memory.
//
// One curator per agent type (replicas=1, strategy=Recreate).
//
// Global memory:  /kord/agents/<name>/memory/global/
// Project memory: /kord/agents/<name>/memory/projects/<project>/
//
...
// memory files change. Project memory is loaded per-job via the runner,
// not via CLAUDE.md.
...
// Only includes global memory. Project memory is injected per-job by the runner.
...
function regenerateClaudeMd() {
  ...
  // Global memory files only
  if (existsSync(GLOBAL_DIR)) {
    try {
      for (const f of readdirSync(GLOBAL_DIR).filter(f => f.endsWith('.md')).sort()) {
        imports.push(`@memory/global/${f}`);
      }
    } catch {}
  }
...
async function processBatch() {
  ...
  for (const [key, updates] of pendingUpdates) {
    try {
      ...
      const changed = await mergeIntoFile(absPath, updates);
      if (changed && scope === 'global') {
        globalChanged = true;
        await maybeQueueConsolidation(absPath, scope);
      }
    } catch (e) {
  ...
  // Consolidate bloated global files, then regen CLAUDE.md
  await runConsolidations();
...
// ─── Consolidation ───
// Periodically rewrite global memory files for coherence.
// Triggered when a file exceeds a line threshold after merge.

const CONSOLIDATION_THRESHOLD = 100; // lines before triggering consolidation
let consolidationQueue = []; // file paths to consolidate after batch

async function maybeQueueConsolidation(absPath, scope) {
  if (scope !== 'global') return;
  try {
    const content = readFileSync(absPath, 'utf8');
    const lines = content.split('\n').filter(l => l.trim() && !l.startsWith('#') && !l.startsWith('---')).length;
    if (lines >= CONSOLIDATION_THRESHOLD) {
      consolidationQueue.push(absPath);
    }
  } catch {}
}
...
async function runConsolidations() {
  ...
  for (const absPath of consolidationQueue) {
    try {
      const content = readFileSync(absPath, 'utf8');
      log('info', 'consolidation_start', { file: basename(absPath), lines: content.split('\n').length });
      ...
      const response = await anthropic.messages.create({
        ...
        messages: [{
          role: 'user',
          content: `You are a memory consolidation agent. Rewrite this memory file to be coherent and concise.

Rules:
- Merge overlapping or related entries into single, comprehensive entries
- Remove contradictions (keep the more specific/recent version)
- Remove redundant entries
- Keep the same format: markdown with "- " bullet entries grouped under "## <date>" sections
- Consolidate old date sections into fewer sections (keep the most recent date for merged entries)
- Preserve all unique, non-redundant information
- Keep it under 80 entries

## Current file contents
${content}

Return the rewritten file content. Start with the "# " header line.`,
        }],
      });
      ...
      if (rewritten.startsWith('#')) {
        ...
        log('warn', 'consolidation_skip_bad_format', { file: basename(absPath) });
      }
    } catch (e) {
      log('error', 'consolidation_failed', { file: basename(absPath), error: e.message });
    }
  }

...
# Edit config.yaml with your cluster IPs, registry, namespaces
sudo ./setup.sh
```
...
```bash
# Hydrate manifests from config.yaml (re-run after config changes)
...
Start from `config.yaml.template` and fill in your values.

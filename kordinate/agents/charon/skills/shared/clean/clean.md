...
Shared procedure — referenced by `/roll clean` and `/migrate`.

Delete PVCs and data in an environment. Destructive — data cannot be recovered.

## Arguments

Required format: `<project> <env>` (e.g., `logbd test`)
...
Pods must be stopped first (`/roll stop`). This procedure will refuse to run if pods are still active in the target namespace.

## Steps

1. Parse project and env. Discover manifests at `<project-repo>/manifests/` and get cluster info from `profile/config.yaml`.
...
2. SSH to the cluster. **Verify pods are stopped**:
   ...
   ```
   If any app pods are running (or infra pods when `--include-infra`), refuse and report: "Stop pods first with /roll stop".

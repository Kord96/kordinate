...
- Gateway Prometheus: cluster-defined retention, short-term buffer only.
- Gateway Loki: 30d retention, 16MB/s ingestion rate limit.
- Tailscale sidecar: exposes Prom:9090, Loki:3101, K8s API:6443 over Tailscale mesh.

**Tier 2 — Master Aggregator**
- Master Alloy: pulls /federate from all cluster Proms via Tailscale (:9090). Pulls /federate from all cluster Lokis via Tailscale (:3101). Writes to Master Prom and Master Loki.
...
```
Pod metrics → Gateway Alloy scrapes → Gateway Prom → Master Alloy federates (:9090) → Master Prom (30d) → Grafana
Pod logs    → Gateway Alloy tails   → Gateway Loki (30d) → Master Alloy federates (:3101) → Master Loki (30d) → Grafana
```
...
| Gateway Prometheus | monitor | vandc | cluster-defined retention local buffer |
...
| Vitals | prod | vandc | Alert evaluation |
...
|---------|-------------|---------------|
| vandc | 100.107.8.117 | Prom:9090, Loki:3101, K8s API:6443 |
...
2. **Vitals does not check MinIO health**: no MinIO evaluation section exists.
...
- **Short local retention (cluster-defined)**: clusters are not storage targets; master owns long-term data.

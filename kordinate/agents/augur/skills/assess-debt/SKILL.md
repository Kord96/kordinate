...
`$ARGUMENTS` — Required: `<project>` (project name or absolute path). If `<project>` is an absolute path, use it directly. Otherwise check `~/<project>/`, `~/repos/<project>/`, `~/test-repos/<project>/`.

## Dependency: detect-concepts or detect-patterns

Reads `<project-repo>/.kord/agents/augur/memory/concepts.md` (from `/detect-concepts`) or `<project-repo>/.kord/agents/augur/memory/patterns.md` (from `/detect-patterns`). Prefers `concepts.md` if both exist. If neither file is present, step 3 falls back to a quick scan.
...
3. **Determine applicable patterns** — check for `<project-repo>/.kord/agents/augur/memory/concepts.md` first, then `patterns.md` as fallback.
   - **If present** (preferred — detect-concepts/detect-patterns does a thorough multi-pass scan):
...
4. **Load anti-patterns** — two sources of anti-patterns to load:
   ...
   **b) From already-confirmed anti-patterns:** for each entry carried forward from step 3.2, read the anti-pattern's concept file from `~/.kord/agents/augur/memory/concepts/<anti-pattern>/concept.md` and check for a `## Impact` section. The Impact section is a prose sentence describing consequences, not an explicit severity keyword. Map it to a severity level using these heuristics:
   ...
   If the `## Impact` section is absent, fall back to the `Confidence` column from the detection report table as a proxy: high = CRITICAL, medium = RECOMMENDED, low = MINOR. These entries skip scanning and go directly into the violations list.
...
5b. **Cross-cutting debt scan** — regardless of which patterns were detected, check for these universal debt indicators. These run independently and catch debt that isn't tied to any specific pattern:

   | Check | What to grep for | Severity |
   |-------|-----------------|----------|
   | Hardcoded secrets/credentials | `password=`, `secret=`, `api_key=` with string literals (not env vars) | CRITICAL |
   | TODO/FIXME/HACK/XXX markers | `TODO`, `FIXME`, `HACK`, `XXX` in source files | MINOR (1 pt each, cap at 5 pts total) |
   | Bare exception catching | `except:` (Python), `catch (Exception` / `catch (e)` without rethrow (Java/JS), `recover()` without logging (Go) | RECOMMENDED |
   | Missing error handling on external calls | HTTP clients, DB queries, or file I/O without try/except/catch around them | RECOMMENDED |
   | Dead code | Unused imports (if linter output available), unreachable branches, commented-out code blocks (>10 lines) | MINOR |
   | Hardcoded config | URLs, ports, hostnames, timeouts as string/number literals rather than config/env | RECOMMENDED if production-facing values; MINOR if development defaults |
   | Missing health endpoints | For long-running servers: absence of `/health`, `/healthz`, `/readyz`, or equivalent | RECOMMENDED |
   | No structured logging | Using `print()` for logging (Python), `console.log` in production code (JS), `fmt.Println` for errors (Go) instead of a logging framework | MINOR |

   Add any findings from this step to the violations list alongside step 4-5 results. These items appear in the report under a "Cross-cutting" category.
...
10. **Write the report** to `<project-repo>/.kord/agents/augur/memory/debt-assessment.md`:

   ```markdown
   # <project> — Tech Debt Assessment

   > Auto-generated by /augur:assess-debt. Last run: <date>
   > Pattern source: concepts.md (full scan) | patterns.md (legacy scan) | quick scan (limited)

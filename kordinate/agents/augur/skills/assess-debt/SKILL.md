...
Reads `<project-repo>/.kord/agents/augur/memory/patterns.md` written by `/detect-patterns`. If that file is absent, step 3 falls back to a quick scan.
...
3. **Determine applicable patterns** — check for `<project-repo>/.kord/agents/augur/memory/patterns.md`.

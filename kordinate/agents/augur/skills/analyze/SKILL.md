...
Write the draft atlas to `/tmp/atlas-draft.json`, then feed it with key source files to Gemini via stdin:
```bash
...
```
Continue immediately. Do NOT use `@$ROOT/` — it fails on large repos.

### Step 4 — Write atlas.json

Assemble into [atlas-schema.md](atlas-schema.md) v4 format. Set `version: "4"` and `generated` to today. Incorporate valid Gemini critiques if available. Write to `$ROOT/.kord/agents/augur/memory/atlas.json`.

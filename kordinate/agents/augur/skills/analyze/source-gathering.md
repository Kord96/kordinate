...
For richer relationship detail (what flows, transport, direction), use `flows` (with the appropriate type: data, control, event, state, resource) and `events` rather than encoding it in `depends_on`.

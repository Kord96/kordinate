...
description: >
  Continuously improve concept detection by cloning diverse repos, running /analyze --deterministic-only,
...
Invokes `/analyze --deterministic-only` via an augur subagent on each sampled repo (step 3). The analyze skill must be installed and functional. Results are read from each repo's `$MEM/atlas.json` (the `concepts` section).
...
2. **Detect** -- run /analyze --deterministic-only on each
...
3. **Run detection** on each cloned repo. Use an Agent subagent with `subagent_type=augur`:
   ```
   Agent(subagent_type="augur", prompt="Run /analyze --deterministic-only on /tmp/train-repos/<repo>")
   ```
...
- **detection fails (analyze --deterministic-only):** Record the failure in the scorecard, continue with next repo.

#!/usr/bin/env python3
"""Export normalized call-edge records from a repo using Joern."""
...
QUERY_SCRIPT = ROOT / "queries" / "export_call_edges.sc"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export call edges from a repo using Joern.")
...
def parse_rows(stdout: str) -> list[dict[str, object]]:
    ...
    for raw_line in stdout.splitlines():
        ...
        for key, value in zip(FIELDS, parts, strict=True):
            if key in {"caller_line", "line_number", "column_number"}:
                try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        stdout = run_joern_query(repo, language, QUERY_SCRIPT, "export_call_edges.sc", args.image_tag)

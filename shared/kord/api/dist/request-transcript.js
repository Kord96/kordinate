...
function extractAgentMessage(payload) {
    ...
    if (typeof payload.message === 'string')
        return summarizeValue(payload.message);
...
export function coalesceTranscriptEvent(previous, next) {
    if (!previous)
        return false;
...
export function buildTranscriptEventFromGateway(requestId, eventRecord) {
...
export function buildTranscriptEventFromProgress(message) {
...
export function buildFinalTranscriptEvent(requestId, agent, status, output) {
    return {
        type: 'result.final',

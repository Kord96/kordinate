...
async function persistCompletedRequests(): Promise<void> {
  await completedRequestStore.save(completedRequests)
}

function hasCompletedRequest(correlationId: string): boolean {
  return completedRequests.has(correlationId)
}

function markCompletedRequest(correlationId: string, status: string): void {
...
async function publishProgress(message: RequestMessage, event: ProgressEventPayload): Promise<void> {
...
function buildRuntimePromptRequest(
  ...
  const promptMessage: RequestMessage = {
    ...message,
...
function getWeakModelAnalysisContext(message: RequestMessage): WeakModelAnalysisContext | undefined {
  const analysisContext = message.agent_params?.analysis_context
  if (!analysisContext || typeof analysisContext !== 'object' || Array.isArray(analysisContext)) return undefined
  const record = analysisContext as Record<string, unknown>
  if (record.execution_strategy !== 'staged-weak') return undefined
  return {
...
function buildWeakModelSummaryPassPrompt(message: RequestMessage, context: WeakModelAnalysisContext): string {
  const parts = [
...
function buildWeakModelWritePassPrompt(message: RequestMessage, context: WeakModelAnalysisContext): string {
  const parts = [
...
async function readWeakModelGroundingSummary(pathValue: string | undefined): Promise<string | undefined> {
  if (!pathValue || !(await pathExists(pathValue))) return undefined
  try {
...
function groundingSummaryLooksPopulated(summary: string | undefined): boolean {
  ...
  if (!requiredSignals.every(signal => summary.includes(signal))) return false
...
async function executeWeakModelStagedAnalysis(
  session: SessionState,
  message: RequestMessage,
  context: WeakModelAnalysisContext,
): Promise<{ session: SessionState; result: RuntimeResult }> {
  ...
  const summaryMessage: RequestMessage = {
    ...message,
  ...
  const writeMessage: RequestMessage = {
    ...message,
  ...
  const writeResult: RuntimeResult = mergedUsage
    ? {
        ...writeRun.result,
        metadata: {
...
function updateSessionPromptCache(session: SessionState, request: RuntimeRequest, result: RuntimeResult): SessionState {
  if (result.status === 'error' || !request.promptPlan?.cacheKey) return session
  return {
    ...session,
...
async function ensureGitSafeDirectory(repoPath: string | undefined): Promise<void> {
  if (!repoPath) return
...
async function findLatestAnalysisDir(analysisRoot: string): Promise<string | undefined> {
  try {
...
async function resolveValidationTargetDir(message: RequestMessage): Promise<string | undefined> {
  const workflowContext = await agentWorkflowHooks?.validationContext?.(message)
  if (workflowContext?.targetDir) return workflowContext.targetDir
  ...
  if (!homeDir || !workingDir) return undefined
  ...
  const isAugurAnalyzeValidator =
    validatorScript.endsWith('/skills/analyze/scripts/validate_output.py')
  ...
  if (isAugurAnalyzeValidator || agentContract.specialization === 'augur') {
    ...
    if (sha && commitTime) {
      ...
      if (await pathExists(expectedDir)) return expectedDir
...
async function runValidatorScript(
  ...
  return await new Promise<ValidationRunResult>((resolve) => {
...
async function runFinalizeScript(
...
function buildArtifactsMetadata(targetDir: string, finalizePayload?: Record<string, unknown>): NonNullable<NonNullable<RuntimeResult['metadata']>['artifacts']> {
...
async function maybeRunValidationLoop(
  session: SessionState,
  message: RequestMessage,
  result: RuntimeResult,
): Promise<{ session: SessionState; result: RuntimeResult }> {
  const validation = agentContract.validation
  if (!validation?.required) return { session, result }
  if (result.status !== 'success') return { session, result }

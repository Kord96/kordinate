...
function genericRuntimeProfile() {
    return {
...
test('injected contract loader reads AGENT_CONTRACT_JSON', () => {
    ...
    try {
        ...
        if (previous === undefined)
            delete process.env.AGENT_CONTRACT_JSON;
        else
            process.env.AGENT_CONTRACT_JSON = previous;
    }
});
test('injected runtime profile loader reads RUNTIME_PROFILE_JSON', () => {
    ...
    try {
        ...
        if (previous === undefined)
            delete process.env.RUNTIME_PROFILE_JSON;
        else
            process.env.RUNTIME_PROFILE_JSON = previous;
    }
});
